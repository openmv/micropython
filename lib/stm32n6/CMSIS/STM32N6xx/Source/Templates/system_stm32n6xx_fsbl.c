...
void SystemInit(void)
{
...
#if defined(STM32N6XX_SI_CUT1_0)
  /*--------- Begin patch for closed cut1.0 sample (bootrom failure) ---------*/

  /* Deactivate RISAF3 AXISRAM2 protection */
  RISAF3->REG[0].CFGR = 0;
  RISAF3->REG[1].CFGR = 0;
  RISAF3->REG[2].CFGR = 0;
  RISAF3->REG[3].CFGR = 0;
  ...
  RISAF3->REG[5].CFGR = 0;
  RISAF3->REG[6].CFGR = 0;
  /* Deactivate SAU & clear 8 regions */
  TZ_SAU_Disable();
  SAU->RNR = 0;
  SAU->RBAR = 0;
  SAU->RLAR = 0;
  ...
  SAU->RBAR = 0;
  SAU->RLAR = 0;
  /* Deactivate MPU & clear 16 regions */
  ARM_MPU_Disable();
  ARM_MPU_ClrRegion(0U);
  ARM_MPU_ClrRegion(1U);
  ARM_MPU_ClrRegion(2U);
  ARM_MPU_ClrRegion(3U);
  ARM_MPU_ClrRegion(4U);
  ARM_MPU_ClrRegion(5U);
  ARM_MPU_ClrRegion(6U);
  ARM_MPU_ClrRegion(7U);
  ARM_MPU_ClrRegion(8U);
  ARM_MPU_ClrRegion(9U);
  ARM_MPU_ClrRegion(10U);
  ARM_MPU_ClrRegion(11U);
  ARM_MPU_ClrRegion(12U);
  ARM_MPU_ClrRegion(13U);
  ARM_MPU_ClrRegion(14U);
  ARM_MPU_ClrRegion(15U);
  /* Deactivate I & Dcache */
  SCB_DisableICache();
  SCB_DisableDCache();
  /* Re-enable interrupts disabled in startup_stm32YYYxx.s */
  __enable_irq();
  /*--------- End patch for closed cut1.0 sample (bootrom failure) -----------*/
  ...
  /* Re-enable interrupts disabled in startup_stm32YYYxx_fsbl.s */
  __enable_irq();

  /*----------End patch Cut1.x sample ----------------------------------------*/
#endif /* STM32N6XX_SI_CUT1_0 */
