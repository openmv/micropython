...
def copy_recursively(vfs, src_dir, print_prefix, mpy_cross):
    ...
    for name in dir_contents:
        ...
        if st[0] & DIR:
            ...
            vfs.opendir(name)
            copy_recursively(vfs, src_name + "/", print_prefix + print_recurse, mpy_cross)
            vfs.closedir()
        else:
            # A file.
            did_mpy = False
...
def make_romfs(src_dir, *, mpy_cross):
    if mpy_cross and mpy_cross_run is None:
        print("Warning: `mpy_cross` module not found, .py files will not be precompiled")
        mpy_cross = False
    ...
    print("/")
    try:
        copy_recursively(vfs, src_dir, "", mpy_cross)
