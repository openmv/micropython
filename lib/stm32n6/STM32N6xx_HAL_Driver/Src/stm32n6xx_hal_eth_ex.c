...
/**
  ...
  * @retval None
  */
void HAL_ETHEx_GetUserTagPriorityQueue(const ETH_HandleTypeDef *heth, ETH_PSRTypeDef *psrq, uint32_t queue)
{
  uint32_t ppsrq;

  if (queue == ETH_RX_QUEUE0)
  {
    ppsrq = READ_BIT(heth->Instance->MACRXQC2R, ETH_MACRXQC2R_PSRQ0_Msk);
    if (ppsrq <= ETH_RX_QUEUE_PRIO_7)
    {
      *psrq = (ETH_PSRTypeDef)ppsrq;
    }
    else
    {
      *psrq = ETH_RX_QUEUE_PRIO_0;
    }
  }
  else if (queue == ETH_RX_QUEUE1)
  {
    ppsrq = READ_BIT(heth->Instance->MACRXQC2R, ETH_MACRXQC2R_PSRQ1_Msk) >> 8;
    if (ppsrq <= ETH_RX_QUEUE_PRIO_7)
    {
      *psrq = (ETH_PSRTypeDef)ppsrq;
    }
...
/**
  ...
  * @param  heth: pointer to a ETH_HandleTypeDef structure that contains
  *         the configuration information for ETHERNET module
  * @param  psrq: pointer to a ETH_PSRTypeDef structure that contains
  *         the configuration of the MAC.
  * @param  queue: queue index
  * @retval None
  */
void HAL_ETHEx_SetUserTagPriorityQueue(ETH_HandleTypeDef *heth, ETH_PSRTypeDef psrq, uint32_t queue)
{
  uint32_t idx;
  uint32_t tmpreg;
  ...
  for (idx = 0; idx < ETH_MTL_RX_Q_CNT; idx++)
  {
    if (idx != queue)
    {
      ...
      tmpreg = READ_BIT(heth->Instance->MACRXQC2R, (ETH_MACRXQC2R_PSRQ_MASK >> (idx * 8U)));
      if ((tmpreg & (uint32_t)psrq) != 0U)
      {
        tmpreg &= ~(uint32_t)psrq;
        MODIFY_REG(heth->Instance->MACRXQC2R,
                   (uint32_t)(0xFU << (idx * 8U)), (uint32_t)(((uint32_t)tmpreg) << (idx * 8U)));
      }
    }
    else
    {
      MODIFY_REG(heth->Instance->MACRXQC2R, (uint32_t)(0xFU << (idx * 8U)), (uint32_t)(((uint32_t)psrq) << (idx * 8U)));
    }
  }
}
...
{
  uint32_t cmd = ETH_WRITE_OPERATION | ETH_MACIACR_OB | ((uint32_t)(typequeueconf->Address) << 8);
...
{
  uint32_t cmd = ETH_READ_OPERATION | ETH_MACIACR_OB | ((uint32_t)(typequeueconf->Address) << 8);
